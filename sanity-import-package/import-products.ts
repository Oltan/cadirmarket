...
const CONFIG = {
  ...
  token: 'YOUR_SANITY_TOKEN',        // Write yetkili token
...
async function importCategories(products: any[]): Promise<CategoryMap> {
  ...
  const uniqueCategories = [...new Set(products.map(p => p.KATEGORİ).filter(Boolean))]
...
async function importProducts(products: any[], categoryMap: CategoryMap): Promise<void> {
  ...
  for (const product of products) {
    ...
    // Kategori referansı
    const categoryRef = product.KATEGORİ && categoryMap[product.KATEGORİ]
      ? { _type: 'reference', _ref: categoryMap[product.KATEGORİ] }
      : null
    ...
    const productDoc: any = {
      ...
      description: generateMetaDescription({
        ...
        category: product.KATEGORİ,
      }),
      model: product.MODEL || undefined,
      material: product.MALZEME || undefined,
      coating: product.KAPLAMA || undefined,
      quantity: product.MİKTAR || undefined,
      packaging: product.BİRİM_PAKET || undefined,
      brand: 'Gema Politek',
      sku: code || undefined,
      isActive: true,
    }
