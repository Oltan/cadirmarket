        ...
        <footer className="border-t bg-zinc-50 dark:bg-zinc-900 dark:border-zinc-800">
            <div className="container mx-auto px-4 py-12">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-zinc-500">{t('aboutText')}</p>
                    </div>
                    ...
                    {/* Kurumsal */}
                    <div>
                        <h4 className="font-semibold mb-4">{t('corporate')}</h4>
                        <ul className="space-y-2 text-sm text-zinc-500">
                            <li><Link href="/about" className="hover:text-green-600 transition-colors">{t('aboutUs')}</Link></li>
                            <li><Link href="/products" className="hover:text-green-600 transition-colors">{t('ourProducts')}</Link></li>
                            <li><Link href="/contact" className="hover:text-green-600 transition-colors">{t('contact')}</Link></li>
                            <li><Link href="/privacy" className="hover:text-green-600 transition-colors">{t('privacy')}</Link></li>
                        </ul>
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-3 text-sm text-zinc-500">
                            <li>
                                <a href="tel:03123855814" className="hover:text-green-600 transition-colors flex items-center gap-2">
                                    <Phone className="w-4 h-4 flex-shrink-0" />
                                    <span>0312 385 58 14 <span className="text-xs text-zinc-400">({t('shop')})</span></span>
                                </a>
                            </li>
                            <li>
                                <a href="tel:05322183061" className="hover:text-green-600 transition-colors flex items-center gap-2">
                                    <Phone className="w-4 h-4 flex-shrink-0" />
                                    <span>0532 218 30 61 <span className="text-xs text-zinc-400">(Murat Demirel)</span></span>
                                </a>
                            </li>
                            <li>
                                <a href="https://wa.me/905322183061" target="_blank" rel="noopener noreferrer"
            ...
            <div className="border-t border-zinc-200 dark:border-zinc-800">
                <div className="container mx-auto px-4 py-6">
                    <details className="group">
                        ...
                        <nav className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-400">
                            {categories.map((category, index) => (
                                <span key={category._id}>
                                    <Link href={`/products?category=${category.slug.current}`} className="hover:text-green-600 transition-colors">
                                        {category.title}
                                    </Link>
            ...
            <div className="border-t border-zinc-200 dark:border-zinc-800">
                <div className="container mx-auto px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-zinc-500">
                    <p>&copy; {new Date().getFullYear()} ÇadırMarket. {t('rights')}</p>
                    <p>{t('distributor')}</p>
                </div>
            </div>
        </footer>
